...
It's ok for this table to contain duplicate rows for the same subject. If a subject is assigned to mulitple variants, Eppo will automatically remove them from the analysis.

The following fields are optional to write to your warehouse, not required by Eppo, and provided mainly for convenience: `featureFlag` and `allocation`. When you call a function to get the assignment, the `experiment` key is generated by combining the `featureFlag` and `allocation` fields. If `allocation` is the key of an experiment allocation, then the assignment data within this experiment key is valid for experiment analysis by Eppo.
