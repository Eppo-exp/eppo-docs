...
| `timestamp` (float)       | The time when the subject was assigned to the variation (Unix timestamp in seconds)                                      | 1624380112.0                        |
| `subjectAttributes` (map) | A free-form map of metadata about the subject                                                                           | `{ "country": "US" }`               |
| `featureFlag` (string)    | An Eppo feature flag key                                                                                                 | "recommendation-algo"               |
| `allocation` (string)     | An Eppo allocation key                                                                                                   | "allocation-17"                     |
| `sdkMetadata` (map)       | Metadata about the SDK instance and configuration                                                                        | `{ "sdkVersion": "1.2.3" }`         |
| `extraLogging` (map)      | Additional context-specific logging data provided during assignment                                                      | `{ "pageViewId": "abc123" }`         |
