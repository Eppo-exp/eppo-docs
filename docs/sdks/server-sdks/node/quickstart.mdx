...
The Eppo Node.js SDK enables feature flags and experiments in your JavaScript applications with only a few lines of code.
...
The Eppo Node SDK is available via a number of package managers.
...
- **Default Value**: Fallback value if the flag is disabled, the assignment fails, or rules don't match
...
:::note
See more details about assignment functions in the [Assignments](/sdks/server-sdks/javascript/assignments) page.
:::
...
if (variation === 'new-algorithm') {
  algorithm = ALGORITHM.BIASED_RANDOM_WALK;
} else {
...
While feature flags are useful, they don't provide a mechanism to measure the impact of the feature. Experiments provide a way to collect data about these interactions using whichever logging and data warehousing system you prefer.
...
const assignmentLogger: IAssignmentLogger = {
  logAssignment(assignment) {
    console.log("Send information to data warehouse:", assignment);
  }
};
...
if (variation === "landing-page-a") {
    return handleLandingPageA();
} else if (variation === "landing-page-b") {
    return handleLandingPageB();
} else {
    return handleLandingPageDefault();
}
```
