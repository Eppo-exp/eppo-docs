...
Before exiting your application, it is recommended to gracefully shutdown the Eppo client. This ensures that it closes any open resources and shuts down the background thread, preventing any scary (but harmless) stacktraces in your logs.

```python
eppo_client.get_instance().shutdown()
```
