...
The SDK will invoke the `log_bandit_action` method with a `bandit_action` hash containing the following fields:
...
To query the bandit for an action, use the `get_bandit_action` method:
...
- `action` (String or nil): The action that was assigned to the subject
