...
let poller_thread = client.start_poller_thread().unwrap();

// Optional: wait 5 seconds for the poller to fetch configuration before serving assignments.
let _ = poller_thread.wait_for_configuration_timeout(Duration::from_secs(5));
```
