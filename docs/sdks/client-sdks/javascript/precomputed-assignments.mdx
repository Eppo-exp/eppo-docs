...
globally distributed to be as close to the user as possible. 
...
On client initialization, you must have the subject key and all available subject attributes available.

## Assignment logging

Using the non-precomputed Eppo client, when a variation is matched, the SDK executes the provided logging callback.

When using the precomputed Eppo client, flag evaluation and assignments occur at different times.

In both case, exposure events are logged when `get*Assignment` is invoked.
