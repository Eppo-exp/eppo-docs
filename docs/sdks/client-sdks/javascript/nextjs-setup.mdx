...
### Turbopack bundling issue (Next.js 16.0.2+)

**Problem:**

If you're using Next.js 16.0.2-canary.7 or later (including 16.0.3+) with Turbopack, you may encounter build errors caused by Turbopack over-aggressively bundling the `thread-stream` package. This includes test files, LICENSE, README, and other non-code assets that should not be bundled.
...
This occurs because Turbopack is incorrectly bundling test files from the `thread-stream` package, which should never be included in your application bundle.

**Why this happens:**

The `@eppo/node-server-sdk` depends on Pino for logging, which uses dynamic imports and `worker_threads`. A fix for [Next.js issue #84766](https://github.com/vercel/next.js/issues/84766) made Turbopack trace worker threads more aggressively, which inadvertently caused [Next.js issue #86458](https://github.com/vercel/next.js/issues/86458) where test files from `thread-stream` get bundled.

**Solution:**

Add the following to your `next.config.ts` (or `next.config.js`) file to treat Pino and its dependencies as external packages:
...
This tells Next.js to treat these packages as external dependencies and not bundle them with Turbopack.

**Note:** Webpack builds are not affected by this issue, as Webpack has been handling dynamic Pino imports correctly for years and properly respects the `package.json` files field.
