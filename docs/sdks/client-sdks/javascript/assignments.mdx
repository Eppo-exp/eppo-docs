...
Contextual bandits are supported, but require using [precomputed assignments](../precomputed-assignments).
...
You can use the assignment logging functions to send data to any data warehouse or logging system you want, including Eppo's [Event Tracking](/sdks/event-logging/event-tracking), or your own custom solution.
...
<TabItem value="eppo" label="Eppo">

```typescript
import { IAssignmentEvent, IAssignmentLogger } from '@eppo/js-client-sdk-common';
import { getInstance, init } from '@eppo/node-server-sdk';

// Define the assignment logger
const assignmentLogger: IAssignmentLogger = {
  logAssignment(assignment: IAssignmentEvent) {
    getInstance().track('eppo_assignment', assignment);
  }
};
