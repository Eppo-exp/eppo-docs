...
There are many options for creating an `EppoJSClient` instance. These options are grouped into different types for some organizational convenience. It is also possible to declare your options as one single object.
At a minimum, the SDK key and an assignment logger must be provided.
...
const eppoClient = EppoJSClient.buildAndInit({
  ...apiOptions,
  ...loggers,
});

// combined options
const allOptions: IClientConfig = {
  apiKey: '<MY SDK KEY>',
  assignmentLogger: { logAssignment: (assignmentEvent) => console.log('Send to warehouse: ', assignmentEvent) },
};

// This client is immediately usable for assignments, however, the default-values will be returned until some configuration
// is loaded. see `waitForConfiguration()`
const eppoClient = EppoJSClient.buildAndInit(allOptions);
...
// Use an Immediately Invoked Function Expression to await the initialization of the client.
export const eppoClient = await (async () => {
  const client = EppoJSClient.buildAndInit({
    ...apiOptions,
    ...loggers,
  });
  // Wait for initialization to complete at whatever point makes sense in your application. The
  // client will return default values until initialization is complete.
...
// Wait for initialization to complete at whatever point makes sense in your application. The
// client will return default values until initialization is complete.
import { eppoClient } from './eppo-init';

export default function myService(): any {

  const useV2 =  eppoClient.getBooleanAssignment('use_v2_backend', 'subject', {}, false);
