...
Precomputed assignments may sometimes be preferrable for performance, e.g. to reduce latency or configuration payload size, and/or privacy, e.g. hiding targeting rules. See [Precomputed Assignments](/sdks/client-sdks/javascript/precomputed-assignments) for more details on how to initialize the SDK with precomputed assignments.
