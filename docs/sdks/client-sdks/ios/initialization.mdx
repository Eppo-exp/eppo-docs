...
:::caution Use separate configuration stores per environment
If your app initializes the SDK in multiple environments (e.g., staging and production) on the same device, each environment must use its own SDK key and configuration store. A single shared configuration cache will be overwritten each time the SDK fetches from a different environment, causing flags to silently flip between staging and production values.
:::
