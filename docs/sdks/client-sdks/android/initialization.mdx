...
If your app initializes the SDK in multiple environments (e.g., staging and production) on the same device, each environment must use its own SDK key and a separate cache location. A single shared cache will be overwritten each time the SDK fetches from a different environment, causing flags to silently flip between staging and production values.
:::
