...
* **Auditability**: Our queries and the resulting tables are fully auditable by your data engineers. Our process rely on a service account whose access you control and can revoke at any time. Your engineers send the token for that service account directly to our secure enclave with no intermediaries. You can update that token on your own if needed.
